...
@interface HSxLiveSession () <LFLiveSessionDelegate>
...
@implementation HSxLiveSession
-(instancetype)initWithDefaultSessionWithdelegate:(id<HSxLiveSessionDelegate>)delegate preView:(UIView *)preView {
    if (self = [super init]) {
        [self setDefaultSessionWithdelegate:delegate preView:preView];
    }
    return self;
}

#pragma mark - 设置默认的Session
- (void)setDefaultSessionWithdelegate:(id<HSxLiveSessionDelegate>)delegate preView:(UIView*)preView{
    if (!_session) {
...
-(void)liveSession:(LFLiveSession *)session liveStateDidChange:(LFLiveState)state{
    ...
    if ([self.delegate respondsToSelector:@selector(liveSession:liveStateDidChange:)]) {
        [self.delegate liveCallBackMessage:needCallBackMessage];
    }
